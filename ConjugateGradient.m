...
for k=1:M
    ...
    %Initialization of Weights, gradients and direction vector.
    W = zeros(N+1,1); %W= Weights matrix for kth output
    g = zeros(N+1,1); %g= Gradient matrix for kth output
    p = zeros(N+1,1); %p= Direction vector 
    
    Egp=1;
    % Loop over iterations for better training. Here the number of
    % iterations is mostly N+1
    for iter = 1:N+1
